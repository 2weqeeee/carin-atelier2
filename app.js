...
const App = {
    ...
    renderUserBadge(user, config) {
        ...
        // Trigger color: always white on carin+ page, else use theme

        const triggerTextColor = isCarinPlusPage ? 'color: white !important;' : 'color: var(--color-text);';



        // Dropdown bg and text must always be readable
        ...
        const menuItem = (href, icon, label, sublabel, extra = '') =>

            `<a href="${href}" onclick="document.getElementById('user-dropdown').style.display='none'"

                style="display:flex; align-items:center; gap:12px; padding:12px 18px; text-decoration:none; color:${dropText}; font-size:13px; font-weight:600; transition:background 0.15s; border-bottom:1px solid ${dropBorder}; ${extra}"

                onmouseover="this.style.background='${hoverBg}'"

                onmouseout="this.style.background='transparent'">

                <span style="font-size:1.3rem; width:24px; text-align:center;">${icon}</span>

                <div>

                    <div>${label}</div>

                    ${sublabel ? `<div style="font-size:11px; color:${dropMuted}; font-weight:400;">${sublabel}</div>` : ''}

                </div>

                ${extra.includes('badge') ? '' : ''}
        ...
        return `
            ...
            <div class="user-badge" id="user-badge-wrap" style="position:relative;">
                ...
                <div id="user-dropdown" style="position:absolute; top:calc(100% + 10px); right:0; background:${dropBg}; border:1px solid ${dropBorder}; border-radius:14px; min-width:260px; box-shadow:0 20px 60px rgba(0,0,0,0.2), 0 4px 16px rgba(0,0,0,0.1); z-index:500; display:none; overflow:hidden; backdrop-filter:blur(8px);">
                    ...
                    <a href="#/carin-plus" onclick="document.getElementById('user-dropdown').style.display='none'"

                        style="display:flex; align-items:center; gap:14px; padding:14px 18px; text-decoration:none; border-bottom:1px solid ${dropBorder}; background: linear-gradient(135deg, ${isDark ? 'rgba(219,39,119,0.15)' : '#fdf2f8'}, ${isDark ? 'rgba(147,51,234,0.12)' : '#fce7f3'}); transition:all 0.2s; position:relative; overflow:hidden;"
                        ...
                        <div>
                            ...
                            <div style="font-weight:800; font-size:13px; background:linear-gradient(90deg,#db2777,#9333ea); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">Descubrir Carin+</div>

                            <div style="font-size:11px; color:${isDark ? '#f472b6' : '#be185d'}; font-weight:500;">Descuentos y beneficios premium</div>

                        </div>

                        <svg style="margin-left:auto; flex-shrink:0;" width="16" height="16" fill="none" stroke="${isDark ? '#f472b6' : '#db2777'}" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
                    ...
                    <div style="display:flex; align-items:center; gap:14px; padding:14px 18px; border-bottom:1px solid ${dropBorder}; background:linear-gradient(135deg, ${isDark ? 'rgba(219,39,119,0.15)' : '#fdf2f8'}, ${isDark ? 'rgba(147,51,234,0.12)' : '#fce7f3'});">
                        ...
                        <div>
                            ...
                            <div style="font-weight:800; font-size:13px; background:linear-gradient(90deg,#db2777,#9333ea); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">Miembro Carin+</div>

                            <div style="font-size:11px; color:${isDark ? '#f472b6' : '#be185d'}; font-weight:500;">Disfrutás todos los beneficios</div>

                        </div>

                        <div style="margin-left:auto; background:linear-gradient(135deg,#db2777,#9333ea); color:white; font-size:9px; font-weight:900; padding:3px 8px; border-radius:999px; text-transform:uppercase; letter-spacing:0.5px; flex-shrink:0;">ACTIVO</div>
            ...
            <div class="product-card" style="${cardStyle} cursor:pointer; min-width:280px;" onclick="if(!event.target.closest('button')) App.navigate('/producto/${p.id}')">
                ...
                <div class="product-info" style="padding:1.5rem;">
                    ...
                    <div style="display:flex; gap:8px; margin-top:auto;">
                        ...
                        <button onclick="App.navigate('/producto/${p.id}')" class="btn btn-default" style="flex:1; font-size:12px;">Ver detalles</button>

                        ${!alreadyOwned ? `<button onclick="App.addToCart('${p.id}')" class="btn btn-dark" style="flex:1.5; font-size:12px;">🛒 Agregar</button>` : ''}
